...
def calculate_time_diff(df):
    ...
    # If picked time is earlier than ordered time → next day
    day_offset = (df['Time_Order_picked'] < df['Time_Orderd']).astype(int)
    ...
    # Fill missing values
    df['order_prepare_time'] = df['order_prepare_time'].fillna(
    ...
    df.drop(
        ['Time_Orderd', 'Time_Order_picked',
         'Time_Ordered_formatted', 'Time_Order_picked_formatted',
         'Order_Date'],
        axis=1,
        inplace=True
    )
